...
$title = [
	"text" => $attributes["title"]["text"] ?? "Atrakcje w okolicy",
...
// Check if $galleryApi is an object before accessing its properties
if ($galleryApi && !empty($galleryApi->media_ids)) {
...
$slidingGallery = [
	...
	"api" => ($galleryApi && $galleryApi->media_ids) ? $galleryApi->media_ids : "", // Fixed: provide empty string instead of array
...
<div <?php echo get_block_wrapper_attributes(); ?> data-wp-interactive="domki-area" <?php echo wp_interactivity_data_wp_context([
		"imageSelected" => 1,
		"showCarousel" => false,
	...
	data-wp-watch="callbacks.handleMore">
	...
	<div class="grid-3 gap-4 py-2 elements container">
		<?php foreach ($attributes["elements"] as $key => $element): ?>
			<div class="area-item">
				<h3><?php echo $element["text"]; ?><small class="unit"><?php echo $element["unit"]; ?></small></h3>
