...
const GalleryImages = ({ gallery, attributes, setAttributes, apiGalleries }: PropsI) => {
  ...
  const saveChanges = async () => {
    ...
    setAttributes({
      ...attributes,
      galleries: attributes.galleries.map((g) => {
        if (g.order_by === gallery.order_by) {
          return newGallery;
        }
        return g;
      }),
    });
    const apiGallery = apiGalleries.find((g) => +g.id === +gallery.gallery_id);

    if (!apiGallery) return;
    const apiGalleryImageIds = apiGallery.media_ids.split(',').map((id) => +id);
    ...
    try {
      ...
      const response = await fetch(wpApiSettings.api_url + 'gallery/' + gallery.gallery_id, {
        method: 'PUT',
        headers: getHeaders(),
        body: JSON.stringify({
